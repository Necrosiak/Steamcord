...
- And the observation that the panel did not look like one panel: he liked the redrawn navigation enough to ask why the voice view and the other menus had not been given the same treatment ([#45](https://github.com/Necrosiak/Steamcord/issues/45)). He was right — one list had been redrawn and everything else still had Steam's native button focus. The whole panel is drawn from one kit as of v1.31.2
- The clip whose sound started a few seconds after the picture, with a recording from before the update and one from after ([#46](https://github.com/Necrosiak/Steamcord/issues/46)). Measuring those two files did not reproduce what the report described — but looking for it did find a real defect no reporter could have known about: a Steam clip is two DASH tracks that do not necessarily start at the same instant, and the mux was normalising each one to zero independently, silently throwing that gap away. Small enough to be invisible on the clips tested here; the exact shape of his symptom on a recording where Steam attaches the audio late
...
- And the logs he sent after v1.31.1 found something nobody was looking for. Three of the five showed him running 1.30.0 while being asked to test 1.31: the automatic update was giving up about fourteen seconds after boot, because Decky's frontend had not loaded yet, and never trying again for the rest of the session. So several of the releases aimed at his problem were never the thing he tested. That is fixed in v1.31.2, and it affected everyone whose loader came up a little late — not only him. The same logs also killed the explanation he had just been given for the missing Go Live audio, by showing the source being selected and confirmed ([#42](https://github.com/Necrosiak/Steamcord/issues/42))
- And the log attached to a three-word answer — "Nope still no sound" — is what finally explained the missing Go Live audio, after five releases had guessed at it. It listed the share dialog's audio options in full, and the venmic dump beside them named the process that had been picked: `opengamepad-ui.x86_64`. That machine is not on Steam's Gamepad UI at all, it runs Open Gamepad UI, and Steamcord's filter — which knew to skip Vesktop, Chromium and the screen reader — had no idea session shells existed. So the filter took the desktop shell for the game and shared a process that makes no sound, while excluding the one entry in that list which actually carried the game. Nothing about it was visible from this side; only the log had it. It also exposed the fault underneath, which was ours and hit everyone: Steamcord's own `enumerateDevices` patch was filtering out the exact device Vesktop looks up to attach the share's audio. The answer was not a better picker but no picker at all — Steamcord now supplies the share's audio itself, so what goes out is the game rather than whichever entry the dialog happened to list ([#42](https://github.com/Necrosiak/Steamcord/issues/42))
...
### [@P1XEL0711](https://github.com/P1XEL0711)

- Vesktop crashing within a minute, every time, in desktop mode on CachyOS ([#49](https://github.com/Necrosiak/Steamcord/issues/49)). The report carried both the Vesktop output and the plugin's journal, and between them they proved the plugin was the culprit twice over: the crash signature was Electron losing its own GPU process — because our `pkill` pattern matched Electron's children — and the log line right underneath showed us aiming at `wayland-0` while his session declared `wayland-1`. He also read that line himself and pointed at the socket. Nothing in Game Mode could ever have surfaced either fault.
