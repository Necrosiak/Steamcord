...
async def setup_discord_tab(tab: Tab):
    await tab.open_websocket()
    await tab.enable()
    # Keep the tab "active" so Chrome doesn't throttle JS/audio in the hidden BrowserView
    await tab._send_devtools_cmd({
        "method": "Emulation.setFocusEmulationEnabled",
        "params": {"enabled": True}
    }, False)
    # Grant mic/camera capture permission BEFORE Discord loads, so its MediaEngine
    # initializes with capture access (granting it at runtime was too late — the engine
    # only ever saw a "default" device and captured nothing → nobody could hear the user).
    await tab._send_devtools_cmd({
