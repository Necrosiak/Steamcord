# Steamcord

**Discord en el Modo Juego de Steam** — un plugin de [Decky Loader](https://github.com/SteamDeckHomebrew/decky-loader) para Steam Deck / Bazzite / SteamOS.

Chat de voz, mensajes directos y compartir pantalla desde el menú de acceso rápido, sin salir del modo juego y sin un segundo dispositivo.

🌍 **Idiomas:** [English](../README.md) · [Français](README.fr.md) · [Deutsch](README.de.md) · **Español** · [Italiano](README.it.md) · [Português](README.pt.md) · [Nederlands](README.nl.md) · [Polski](README.pl.md) · [Русский](README.ru.md)

> **Steamcord es un proyecto independiente.** Originalmente se inspiró en
> [Deckcord](https://github.com/marios8543/Deckcord) (ver Créditos), pero el código se ha
> reescrito en gran parte y ahora sigue su propia dirección — no está afiliado ni respaldado
> por ese proyecto.
>
> La interfaz está totalmente traducida a 9 idiomas y sigue automáticamente el idioma de SteamOS.

---

## Cómo funciona

Steamcord ejecuta **[Vesktop](https://github.com/Vencord/Vesktop)** — un cliente de Discord nativo de verdad — invisible en segundo plano, y lo controla mediante el Chrome DevTools Protocol. El plugin le inyecta un pequeño cliente y expone todo en el **menú de acceso rápido** de Steam.

Pasar a nativo resuelve los problemas difíciles del antiguo enfoque de navegador oculto: **tu micrófono y el audio de voz funcionan de forma nativa**, igual que en la app de escritorio de Discord — sin trucos de captura ni rodeos de reproducción automática. Vesktop se inicia (y se instala si falta) automáticamente, mantiene la sesión tras reiniciar y nunca necesita una ventana de escritorio en el Modo Juego.

---

## Funciones

- **Un Discord por cuenta de Steam (multisesión)** — Cada usuario de Steam de la máquina tiene **su propio perfil de Discord**: cambia de cuenta de Steam y Steamcord cambia de Discord automáticamente en segundos (la primera vez muestra el login QR; después cada sesión queda recordada). Nadie acaba en el Discord de otra persona.
- **Inicio de sesión con código QR** — Escanea un código QR con la app móvil de Discord para entrar al instante. En tu teléfono: *Discord → Ajustes → Escanear código QR*, luego apunta al código del panel. Sin escribir contraseñas en la Deck.
- **Resolver el CAPTCHA, sin Modo Escritorio** — Discord a veces pone un CAPTCHA en su página de inicio de sesión; hasta que se resuelve no emite ningún ticket de acceso y el código QR se regenera sin fin. Steamcord lo detecta y refleja la página de inicio de sesión en Modo Juego para que resuelvas el desafío con el mando: la cruceta mueve el puntero, A hace clic.
- **Navegación unificada** — Pestañas **Voz / Texto / ⚙️ Ajustes** arriba, con un selector **Servidores / MD** compartido debajo: el mismo interruptor de fuente sirve para la voz y el texto.
- **Chat de voz** — Únete a canales de voz y escucha a todos, con cada miembro mostrado en vivo (anillo al hablar, distintivos de silencio/ensordecer), un control de volumen por persona (0–200 %) **y un silencio local por persona** (silencia a alguien solo para ti, sin que lo sepa). Micrófono y audio nativos (Vesktop).
- **Mensajes directos (MD y grupos)** — Explora tus conversaciones e inicia/únete a llamadas de voz con amigos directamente desde el menú de acceso rápido. Las llamadas activas se resaltan.
- **Explorador de voz de servidores** — Mira qué canales de voz tienen gente (con avatares) antes de unirte.
- **Chat de texto — servidores *y* MD** — Lee y responde a un canal de servidor **o a una conversación privada** desde el QAM (campo a ancho completo, el teclado de Steam se abre solo). **Las imágenes adjuntas se muestran como miniaturas** (cargadas solo mientras el canal está abierto) y **los enlaces se abren en el navegador del Modo Juego**. Desplazamiento automático al último mensaje.
- **Estado de Discord en tu nombre** — Tu **nombre de usuario clicable** arriba muestra tu estado actual; tócalo para cambiarlo. Una sincronización automática opcional hace que Discord **siga tu estado de Steam** en segundo plano; elegir un estado a mano vuelve al modo manual.
- **Selección de dispositivos de audio** — Desde Ajustes, elige el dispositivo de **salida (sonido de Discord)** y de **entrada (micrófono)** — *Auto (predeterminado del sistema)* o uno concreto, p. ej. enviar el sonido de Discord solo a tus **auriculares** mientras el juego sigue por HDMI. Un botón **Recargar dispositivos de audio** hace que el sistema y Discord vuelvan a leer los dispositivos — para un micrófono que no se capta en el modo Juego.
- **Silenciar / Ensordecer / Desconectar** — Controles de voz con un toque desde el QAM.
- **Compartir pantalla** — Comparte toda tu pantalla en un canal de voz (Go Live), de forma nativa en Escritorio, Big Picture y Modo Juego. En Modo Juego Steamcord proporciona un pequeño portal ScreenCast (`portal_shim.py`) que entrega a Chromium el nodo PipeWire de gamescope usado por la grabación de Steam. El audio del juego pasa por un puente aislado, por lo que el audio de las llamadas de Discord nunca vuelve al directo. No requiere módulo de kernel ni escribir en el rootfs. El relé WebRTC de GStreamer y el botón de cámara virtual **«Compartir pantalla (modo juego)»** (v4l2loopback) siguen como alternativas.
- **Ver un stream mantiene la pantalla encendida** — mientras ves el Go Live de alguien (en el panel, la vista ampliada o la cuadrícula a pantalla completa), el salvapantallas de SteamOS se desactiva y luego se restaura tal cual estaba. Se puede desactivar en los Ajustes; el atenuado de pantalla y la suspensión automática no se tocan nunca.
- **Compartir el audio del juego** — Transmite el sonido de tu juego al canal de voz **junto con tu voz**. Dos controles de mezcla (🎙️ voz / 🎮 juego) definen lo que oyen los demás, mientras tú sigues oyendo el juego con normalidad — y funciona incluso **sin micrófono físico** (el plugin crea una entrada virtual *Steamcord Mic*).
- **Notificaciones en el juego** — Las llamadas de MD entrantes y las menciones aparecen como **notificaciones nativas de Steam (popup + sonido)**, respetando tu estado de Discord (silenciadas en invisible / no molestar). Un mensaje suena por los dos lados a la vez (el sonido de Discord y el de Steam): los ajustes de notificaciones permiten elegir cuál conservas — Discord por defecto.
- **🕹️ Atajo de mando para la voz** — Captura **cualquier combinación de botones de tu mando** y asígnala a **silenciar (alternar)** o **pulsar para hablar**. Funciona globalmente dentro del juego, incluso con el QAM cerrado (se configura en la pestaña de Ajustes).
- **Enviar capturas** — Envía una captura de Steam directamente a la conversación que tengas abierta.
- **[Vencord](https://vencord.dev/)** está integrado en Vesktop, dando acceso a su ecosistema de plugins.
- **Guardar archivos adjuntos** — Cualquier archivo publicado en una conversación puede guardarse en su carpeta de Descargas: imágenes, vídeos y todo lo demás (PDF, archivos comprimidos, registros) que antes solo aparecía como un número. Las imágenes y los vídeos se abren a pantalla completa en el panel, no en el navegador de Steam.
- **Reenviar mensajes** — Enviar un mensaje, con sus imágenes y archivos, a otro canal o mensaje directo sin salir del juego. El nombre del autor lo acompaña.
- **Eventos programados** — Ver qué está por llegar en sus servidores, abrir un evento para su imagen, su descripción y sus participantes, y unirse directamente al canal de voz de un evento en curso.
- **🖥️ Vista ampliada (prueba de concepto)** — Un Discord a pantalla completa, abierto desde el icono junto a tu nombre: MD con la actividad de cada persona, **amigos** (actividad, escribir a cualquiera, llamar, añadir por nombre de usuario, solicitudes), **servidores** que se despliegan en **miembros por rol**, eventos, canales de texto y de voz con su **chat del canal**, todos los eventos en un sitio, la llamada en curso y los ajustes. Los chats se abren dentro de la vista; **L1 / R1** retroceden y avanzan, **B** sube un nivel: sale de un chat o de los miembros de un servidor y luego devuelve el panel de acceso rápido, que la barra lateral también ofrece como botón. Los ajustes del panel eligen cuál de los dos abre el icono de Steamcord; **Y** sobre una persona abre Perfil, Mensaje, Llamar y las acciones de amistad y bloqueo. Salir de la vista nunca corta una llamada. El oscurecimiento del fondo se ajusta en los ajustes, hasta ocultarlo por completo. *Aún es una prueba de concepto: se esperan errores, los informes son bienvenidos.*
- **Modo streamer** — Mientras estás en directo (Go Live de Discord, o una transmisión o grabación de [BoneCast](https://github.com/Necrosiak/BoneCast)), las notificaciones se retienen para que no salgan en el vídeo y se muestran después. *Siempre activo* cubre OBS y otros programas de streaming.
- **Nombre visible** — El panel muestra tu nombre visible de Discord en lugar de tu usuario, y puedes cambiarlo desde los Ajustes.
- 🐧 **Compatibilidad** — trabajamos activamente para soportar todos los SO capaces de ejecutar Steam en modo juego / Big Picture (Linux por ahora): detección portable, dependencias Python incluidas, sin suposiciones específicas de distribución. Notas por distribución: [OS-NOTES.md](OS-NOTES.md).

---

## 📸 Capturas de pantalla

<p align="center">
  <img src="img/steamcord-qam-servers.jpg" width="49%" alt="Panel de Steamcord en el menú de acceso rápido, lista de servidores"/>
  <img src="img/steamcord-qam-call.jpg" width="49%" alt="Panel de Steamcord durante una llamada de voz"/>
</p>
<p align="center">
  <img src="img/steamcord-expanded-friends.jpg" width="49%" alt="Vista ampliada, amigos en línea con su actividad"/>
  <img src="img/steamcord-expanded-servers.jpg" width="49%" alt="Vista ampliada, lista de servidores"/>
</p>
<p align="center">
  <img src="img/steamcord-expanded-chat.jpg" width="49%" alt="Vista ampliada, chat abierto dentro de la vista"/>
  <img src="img/steamcord-expanded-call.jpg" width="49%" alt="Vista ampliada, llamada en curso"/>
</p>

## Instalación

> **Aún no está en la Decky Store.** Instalación manual mediante el modo desarrollador.

1. Activa el **modo desarrollador** en Decky → Ajustes generales
2. Ve a **Desarrollador** en los ajustes de Decky
3. Instala desde la URL:
   `https://github.com/Necrosiak/Steamcord/releases/latest/download/Steamcord.zip`

Vesktop se instala y se inicia automáticamente con el plugin la primera vez. Solo inicia sesión una vez (código QR, o Vesktop en Modo Escritorio) y permaneces conectado.

### Requisito (compartir pantalla)
La pantalla compartida funciona de inmediato: el complemento instala automáticamente su dependencia de Python (aiohttp) en el primer arranque. GStreamer lo proporciona el sistema.

---

## Compilar desde el código fuente

```bash
git clone https://github.com/Necrosiak/Steamcord
cd Steamcord
pnpm install
pnpm run build
# copia dist/, main.py, defaults/, plugin.json, package.json a ~/homebrew/plugins/Steamcord/
sudo systemctl restart plugin_loader
```

---

## 🐛 Issues e ideas — ¡no lo dudes!

¿Un bug, un comportamiento raro en tu distribución, una función que falta?
**Abre una [issue](https://github.com/Necrosiak/Steamcord/issues)** — cada
informe orienta directamente lo que se construye después. Incluye si puedes:

- tu distribución y versión (Bazzite 42, CachyOS, Ubuntu 24.04…) y cómo corre Steam (modo juego / Big Picture / escritorio)
- la versión del plugin (Ajustes → Actualización) y si Vesktop es flatpak o nativo
- qué hiciste, qué esperabas, qué pasó en su lugar
- logs: `~/homebrew/logs/Steamcord/` y `journalctl -b | grep -i steamcord`

Las peticiones de funciones y los «¡funciona!» en configuraciones inusuales valen igual.

## Créditos

- Proyecto original: [marios8543/Deckcord](https://github.com/marios8543/Deckcord) — arquitectura, BrowserView, compartir pantalla con GStreamer
- [@aagaming](https://github.com/AAGaming00) — soporte de micrófono vía la pestaña SteamClient (relé WebRTC)
- [@Epictek](https://github.com/Epictek) — base del inicio de sesión con QR
- [@jessebofill](https://github.com/jessebofill) — código de parcheo del menú de Steam
- [Vesktop / Vencord](https://github.com/Vencord/Vesktop) — el cliente de Discord nativo que controla Steamcord

## Aviso sobre IA

Se usó IA en la creación de este proyecto: para ingeniería inversa, desarrollo y documentación. **No** se usó para arte ni escritura creativa. Todo lo publicado lo revisa una persona, y el resultado es responsabilidad mía. Si eso te incomoda, ya lo sabes.
