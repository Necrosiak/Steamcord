...
- **🖥️ Vista ampliada (prova de conceito)** — Um Discord em ecrã inteiro, aberto a partir do ícone ao lado do teu nome: MD com a atividade de cada pessoa, **amigos** (atividade, escrever a qualquer pessoa, ligar, adicionar por nome de utilizador, pedidos), **servidores** que se abrem em **membros por cargo**, eventos, canais de texto e de voz com o seu **chat do canal**, todos os eventos num só sítio, a chamada em curso e as definições. Os chats abrem dentro da vista; **L1 / R1** recuam e avançam, **B** sobe um nível: sai de uma conversa ou dos membros de um servidor e depois devolve o painel de acesso rápido, que a barra lateral também oferece como botão. As definições do painel escolhem qual dos dois o ícone do Steamcord abre; **Y** sobre uma pessoa abre Perfil, Mensagem, Ligar e as ações de amizade e bloqueio. Sair da vista nunca termina uma chamada. O escurecimento do fundo ajusta-se nas definições, até o ocultar por completo. *Ainda uma prova de conceito — são esperados bugs, os relatos são bem-vindos.*
- **Modo streamer** — Enquanto estás em direto (Go Live do Discord, ou uma transmissão ou gravação [BoneCast](https://github.com/Necrosiak/BoneCast)), as notificações ficam retidas para não aparecerem no vídeo e são mostradas depois. *Sempre ativo* cobre o OBS e outros programas de streaming.
- **Nome de exibição** — O painel mostra o teu nome de exibição do Discord em vez do login, e podes alterá-lo nas Definições.
