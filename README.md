...
- [@DavidNotProgamer2](https://github.com/DavidNotProgamer2) — report of the untranslated screen-share hint and the chat-style look of Decky notifications ([#2](https://github.com/Necrosiak/Steamcord/issues/2), addressed in v1.14.0), diagnosis of the screen wake-lock with volume-mixer evidence ([#3](https://github.com/Necrosiak/Steamcord/issues/3)) report of the green incoming video, the vanishing Watch button and the volume-slider reset ([#5](https://github.com/Necrosiak/Steamcord/issues/5), addressed in v1.14.2), report of incoming streams missing in group and server voice channels and the mirrored second stream ([#8](https://github.com/Necrosiak/Steamcord/issues/8), addressed in v1.14.4 and v1.14.5), and the shutdown-hang bisect ([#7](https://github.com/Necrosiak/Steamcord/issues/7), fixed in v1.14.5), and the detailed repro of the rapid Go Live toggle breakage and the stuck stream preview on stock SteamOS ([#12](https://github.com/Necrosiak/Steamcord/issues/12), fixed in v1.16.0), and the reports of the phantom volume changes, the mic/keybind/audio settings bugs ([#13](https://github.com/Necrosiak/Steamcord/issues/13), [#14](https://github.com/Necrosiak/Steamcord/issues/14), fixed in v1.16.1), and the post-`chown` `ls -la` on the updater permission bug that showed exactly which files and directories stayed root-owned, pinning down the real cause ([#16](https://github.com/Necrosiak/Steamcord/issues/16), fixed in v1.16.9), and the fullscreen chat concept with hand-drawn mockups — passive live preview in the QAM, a real navigable fullscreen view with auto-follow, and the live testing feedback that shaped it ([#20](https://github.com/Necrosiak/Steamcord/issues/20), shipped in v1.17.0), and the v1.18.0 feedback that shaped v1.18.1 — the in-game overlay toggle not persisting and the missing controller focus ring ([#22](https://github.com/Necrosiak/Steamcord/issues/22)), and the chat report of phantom reaction counts, unrendered custom server emojis and per-channel fullscreen notification behaviour ([#21](https://github.com/Necrosiak/Steamcord/issues/21)), and the `[overlay]` logs that pinned the dead in-game overlays down — first on a missing WebKit2 4.1 binding, then, when that was not enough, on SteamOS shipping no WebKitGTK binding at all, which is what led to the web-engine-free overlay renderer — plus the report of the speaking ring hidden behind portrait streams ([#22](https://github.com/Necrosiak/Steamcord/issues/22), fixed in v1.18.3), and the persistence on the fullscreen scroll and the randomly slow messages — insisting both were still broken after v1.18.1, which is what uncovered that the live feed was being switched off underneath the fullscreen view ([#21](https://github.com/Necrosiak/Steamcord/issues/21), fixed in v1.18.4), and the suggestion that ended the jump-to-latest saga — asking for the button to be replaced by a controller shortcut that puts the selection back on the composer, then spotting that going back up landed one message short ([#21](https://github.com/Necrosiak/Steamcord/issues/21), shipped in v1.19.0), the report that the very first message notification never pops ([#23](https://github.com/Necrosiak/Steamcord/issues/23), fixed in v1.19.0), and the report that only one screen share shows at a time ([#24](https://github.com/Necrosiak/Steamcord/issues/24), fixed in v1.19.0)
...
- [@humzakh](https://github.com/humzakh) — report of the in-plugin updater failing on root-owned installs, including the chmod/chown attempts and the follow-up reports across several fix attempts that eventually pinned down the real cause ([#16](https://github.com/Necrosiak/Steamcord/issues/16), properly fixed in v1.18.2), and report of the broken message-list navigation and missing conversation history ([#17](https://github.com/Necrosiak/Steamcord/issues/17), fixed in v1.16.2)
...
- [@william097y](https://github.com/william097y) — report of games appearing in Discord without their Rich Presence artwork, with the observation that the affected titles all carried a `™`, which is exactly what pointed at Steam and Discord spelling the same game differently, and the follow-up test on a title with no `™` that widened the cause to typographic punctuation in general ([#32](https://github.com/Necrosiak/Steamcord/issues/32), fixed in v1.21.1)
